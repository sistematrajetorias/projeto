...
#EstadoCadeias <- function(mat, estado, menc) {
  ...
  # Cadeia A: CB >> ED >> OA >> BD 
 # if (any(mat == 'BANCOS DE DADOS')) {
  #  estado <- 4  
  #} else if (any(mat == 'Organiza��o de Arquivos') & estado < 3) {
  #  estado <- 3
  #} else if (any(mat == 'ESTRUTURAS DE DADOS ') & estado < 2) {
  #  estado <- 2
  #} else if (any(mat == 'COMPUTACAO BASICA') & estado < 1 ) {
  #  estado <- 1
  #}
  #return(estado)
#}
...
EstadoCadeias <- function(mat, estado, menc) {
  ...
  # Cadeia A: F1 >> F2 >> F3 >> CD >> OAC 
  if (any(mat == 'ORGANIZACAO E ARQUITETURA DE COMPUTADORES')) {
    estado <- 5
  } else if (any(mat == 'CIRCUITOS DIGITAIS') & estado < 4) {
    estado <- 4  
  } else if (any(mat == 'FISICA 3') & estado < 3) {
    estado <- 3
  } else if (any(mat == 'FISICA 2') & estado < 2) {
    estado <- 2
  } else if (any(mat == 'FISICA 1') & estado < 1 ) {
    estado <- 1
  }
  return(estado)
}
...
ExtraiTrajComp <- function(arquivo) {
